...
from torch import cat, square,zeros, clone, abs, sigmoid, float32, tanh, clamp, arange
...
class SageShortLong(Module):
    def __init__(self, hidden_channels, in_channels, out_channels, encode=True, conv_layers=5, conv2_layers = 3, conv_activation='relu', 
                    decode_layers=2, decode_activation='leakyrelu', layernorm=True, variance=1, agg='sum', rho=0):
        super(SageShortLong, self).__init__()
        '''Model built upon the GraphSAGE convolutional layer. This is a node only model (no global, no edge).
        Model takes a data object from a dataloader in the forward call and takes out the rest itself. 
        hidden_channels, n_in, n_out must be specified
        Most other things can be customized at wish, e.g. activation functions for which ReLU and LeakyReLU can be used'''
        ...
        if self.encode:
            self.convs2.append(SAGEConv(hidden_channels, hidden_channels))
        else:
            self.convs2.append(SAGEConv(in_channels, hidden_channels))
        for _ in range(int(conv2_layers-1)):
            self.convs2.append(SAGEConv(hidden_channels, hidden_channels))
        ...
        for _ in range(out_channels):
            ...
            for i in range(decode_layers):
                if i==0:
                    self.norm.append(LayerNorm(normalized_shape=2*hidden_channels)) #for the first layer, double input needed (short/long tree)
                    self.decoder.append(Linear(2*hidden_channels, hidden_channels))
                elif i==decode_layers-1: ## if final layer, make layer with only one output
                    self.norm.append(LayerNorm(normalized_shape=hidden_channels))  
                    self.decoder.append(Linear(hidden_channels, 1))
                else:
                    self.norm.append(LayerNorm(normalized_shape=hidden_channels))
        ...
        if variance:
            ...
            for _ in range(out_channels):
                ...
                for i in range(decode_layers):
                    if i==0:
                        self.sig_norm.append(LayerNorm(normalized_shape=2*hidden_channels))
                        self.sig.append(Linear(2*hidden_channels, hidden_channels))
                    elif i==decode_layers-1:
                        self.sig_norm.append(LayerNorm(normalized_shape=hidden_channels))
                        self.sig.append(Linear(hidden_channels, 1))
                    else:
                        self.sig_norm.append(LayerNorm(normalized_shape=hidden_channels))
        ...
        if self.rho!=0:
            ...
            for _ in range(self.rho):
                ...
                for i in range(decode_layers):
                    if i==0:
                        self.rho_norm.append(LayerNorm(normalized_shape=2*hidden_channels))
                        self.rho_l.append(Linear(2*hidden_channels, hidden_channels))
                    elif i==decode_layers-1:
                        self.rho_norm.append(LayerNorm(normalized_shape=hidden_channels))
                        self.rho_l.append(Linear(hidden_channels, 1))
                    else:
    ...
    def forward(self, graph):
        ...
        #get the data
        x, edge_index, batch = graph['A'].x, graph['A'].edge_index, graph['A'].batch
        x2, edge_index2, batch2 = graph['B'].x, graph['B'].edge_index, graph['B'].batch
